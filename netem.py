...
def netem_make_command(iface, classid, handle, delay=0, jitter=0,
                       delay_corr=0, distribution="", loss=0,
                       loss_corr=0, bandwidth=0):
...
def setup_connections(connections):
    ...
    for conn in connections:
        egress_cmd = netem_make_command(out_iface,
                                        egress_classid,
                                        egress_handle,
                                        delay=conn["latency_up"],
                                        jitter=conn["latency_jitter"],
                                        loss=conn["loss_up"],
                                        loss_corr=conn["loss_jitter"],
                                        bandwidth=conn["speed_up"])
        ingress_cmd = netem_make_command(in_iface,
                                         ingress_classid,
                                         ingress_handle,
                                         delay=conn["latency_down"],
                                         jitter=conn["latency_jitter"],
                                         loss=conn["loss_down"],
                                         loss_corr=conn["loss_jitter"],
                                         bandwidth=conn["speed_down"])
